...
class Data():
    def __init__(self, args):
        ...
        for index in range(len(input_data)):
            ...
            if self.args.model == "llama-2-chat":
                prompt = self.get_prompt_llama2_chat(prompt)
            elif self.args.model == "llama-2":
                prompt = prompt
        ...
        with open(os.path.join(self.args.base_dir, self.args.output_dir), 'w') as fp:
            for line in output_data:
                json.dump(line, fp)
                fp.write('\n')

    def get_prompt_llama2_chat(self, prompt):
...
def main():
    ...
    parser.add_argument('--model', default='llama-2-chat', type=str, help='llama-2 or llama-2-chat')
